...
if $ROLLBACK; then
    ...
    docker compose pull --quiet || true  # re-pull in case we're on latest
...
if $CHECK_ONLY; then
    ...
    docker compose pull --dry-run 2>&1 || docker compose build --dry-run 2>&1 || true
...
CURRENT_ID=$(docker compose images api -q 2>/dev/null | head -1 || echo "")
...
docker compose up -d --no-deps --force-recreate worker worker_dead_letter beat
sleep 5

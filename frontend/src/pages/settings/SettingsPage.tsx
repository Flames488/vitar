...
// Defaults on for every clinic (backend column default=True), but stays a
// real per-clinic choice — a clinic can still switch it off here.
...
function PaymentTogglePanel() {
  ...
  const { register, handleSubmit, formState: { isSubmitting } } = useForm<PaymentToggleFormValues>({
    values: {
      patient_payment_enabled: data?.patient_payment_enabled ?? true,
    },
  });
